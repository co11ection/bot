...
class GoogleSheet:
    ...
    SPREADSHEET_ID = '14dM94rgiKnr3ccPYVTscL4ObH_0jAhd7dhU3-U4Gecc'
    service = None
    ...
    def updateRangleValues(self, range, values):
        data = [{
            "range":range,
            'values':values
        }]
...
def sales_info_to_sheets():
    with open('increment_for_sales.txt', 'r') as f:
        a = int(f.read())
    gs = GoogleSheet()
    today = datetime.date.today().__str__()
    sales_range = f'sales!A{a}:K{a}'
    sales_values = [
        [today,
        FormForSales.deadline,
        FormForSales.number_quantity, 
        FormForSales.quantity_signup,
        FormForSales.quantity_payment,
        FormForSales.quantity_old_dialogues,
        FormForSales.quantity_new_dialogues,
        FormForSales.quantity_of_calls_made_per_day,
        FormForSales.quantity_of_entries_to_trial_lesson,
        FormForSales.quantity_comes_to_trial_lesson,
        ],   
    ]
    gs.updateRangleValues(sales_range,sales_values)
    a = a + 1
    with open('increment_for_sales.txt', 'w+') as f:
        f.write(f'{a}')


def curators_info_to_sheets():
    with open('increments_for_corators.txt', 'r') as f:
        a = int(f.read())
    gs = GoogleSheet()
    today = datetime.date.today().__str__()
    sales_range = f'curators!A{a}:K{a}'
    sales_values = [
        [today,
        FormForKurators.day_or_ev,
        FormForKurators.group,
        FormForKurators.quantity_of_students,
        FormForKurators.comes_quantity,
        FormForKurators.lates_quantity,
        FormForKurators.gone_away,
    ...
    gs.updateRangleValues(sales_range,sales_values)
    a = a + 1
    with open('increments_for_corators.txt', 'w+') as f:
        f.write(f'{a}')


def rating_info_to_sheets():
    with open('increments_for_raing.txt', 'r') as f:
        a = int(f.read())
    gs = GoogleSheet()
    today = datetime.date.today().__str__()
    sales_range = f'eduteam!D{a}:K{a}'
    sales_values = [
        [today,
        Ratings.position,
        Ratings.staff_choise,
        Ratings.weekly_rating,
        ],   
    ]
    gs.updateRangleValues(sales_range,sales_values)
    a = a + 1
    with open('increments_for_raing.txt', 'w+') as f:
        f.write(f'{a}')

def lates_info_to_sheets():
    with open('increments_for_lates.txt', 'r') as f:
        a = int(f.read())
    gs = GoogleSheet()
    today = datetime.date.today().__str__()
    sales_range = f'eduteam!A{a}:K{a}'
    sales_values = [
        [today,
        Lates.quantity_of_lates,
        ],   
    ]
    gs.updateRangleValues(sales_range,sales_values)
    a = a + 1
    with open('increments_for_lates.txt', 'w+') as f:
        f.write(f'{a}')
